...
function ready(error, uk, locations, routes) {
    ...
    function showInfoAll(d) {
       ...
       d3.selectAll(".chart")
        ...
        var routesArrivingLondon = routes.filter(function(d) {
            if (d.lad_arrive_code == code && d.lad_depart_london == 1 &&
                d.lad_depart_code != code && d.lad_depart_name !=
                "Unknown London") {
                return true;
            }
        });
        ...
        var routesUnknownOrOutsideLondon = routes.filter(function(d) {
            if (d.lad_arrive_code == code && d.totalsNotLondon == 1) {
                return true;
            }
        });
        ...
        var routesArrivingLondonAndOutside = routes.filter(function(d) {
            if (d.lad_arrive_code == code && d.routesArrivingLondonAndOutside ==
                1) {
                return true;
            }
        });
        ...
        var routesDepartingLondon = routes.filter(function(d) {
            if (d.lad_depart_code == code && d.lad_depart_london == 1) {
                return true;
            }
        });
        ...
        origin_circles.selectAll("circle")
            ...
            .append("circle")
            .attr("cx", function(d) {
                return projection([+d.lon_depart, +d.lat_depart])[0];
            })
            .attr("cy", function(d) {
                return projection([+d.lon_depart, +d.lat_depart])[1];
            })
            .attr("r", function(d) {
                if (d.count == 1) {
                    return 6;
        ...
        rectangle.selectAll("rect")
            ...
            .append("rect")
            .attr("x", function(d) {
                return projection([+d.lon_depart - 0.05, +d.lat_depart +
                    0.038
                ])[0];
            })
            .attr("y", function(d) {
        ...
        rectangle.selectAll("text")
            ...
            .append("text")
            .attr("x", function(d) {
                return projection([+d.lon_depart - 0.046, +d.lat_depart +
                    0.03
                ])[0];
            })
            .attr("y", function(d) {
        ...
        destination_circles.selectAll("circle")
            ...
            .attr("cx", function(d) {
                return projection([+d.lon_arrive + 0.025, +d.lat_arrive])[0];
            })
            .attr("cy", function(d) {
                return projection([+d.lon_arrive + 0.025, +d.lat_arrive])[1];
            })
        ...
        var heading = d3.select("#infowindowHeading")
            .append("text")
            .html(function(d) {
                if (boroughNameById.get(code) === undefined) {
                    return null;
                } else {
                    return boroughNameById.get(code);
                }
            })
        ...
        var instructionAllJourneys = d3.select("#infowindowHeading")
            ...
            .html(function(d) {
                if (boroughNameById.get(code) === undefined) {
                    return "<span class='tooltip_heading'>Domestic Abuse Migration Map</span> <br><span class='tooltip_text'> Women's journeys to access refuge in London, April 2015 - March 2016<br><span id='instructionAllH1'>Showing all journeys</span><br><b>Roll over</b> the London boroughs to discover the journeys women made.<br><b>Use the buttons above</b> the map to see just those arriving or leaving.</span>";
                } else {
                    return null;
                }
            })
        ...
        var refugeSpaces = d3.select("#totalRefuge")
            .append("text")
            .html(function(d) {
                if (boroughRefugeTotal.get(code) === undefined) {
                    return null;
                } else if (boroughBeds.get(code) < 1) {
                    return "<span id='spaces'>No </span> refuge spaces available";
                } else {
                    return "<span id='spaces'>" + boroughBeds.get(code) +
                        " </span> refuge spaces available";
                }
            })
        ...
        var helplineCalls = d3.select("#helplineCalls")
            .append("text")
            .html(function(d) {
                if (helplineCallsById.get(code) === undefined) {
                    return null;
                    ...
                    return "<span id='helplinecalls'>" + helplineCallsById.get(
                            code) +
        ...
        var arrivalsHeading = d3.select("#arrivalsHeading")
            ...
            .html(function(d) {
                if (boroughRefugeTotal.get(code) === undefined) {
                    ...
                    return "<span id='arrivingH1'>Arriving to access refuge</span>";
                }
            })
            .attr("class", "tooltip_text");
        ...
        var arrivalsInfo = d3.select("#arrivalsInfo")
            ...
            .html(function(d) {
                if (boroughRefugeTotal.get(code) === undefined) {
                    ...
                    return "<span id='arriving'>" + boroughRefugeTotal.get(
                            code) + " </span>" +
                        "  women accessed a refuge in " + boroughNameById.get(
                            code);
                }
            })
            .attr("class", "tooltip_text");
        ...
        var originFromBoroughText = d3.select("#arrivalsInfo")
            ...
            .html(function(d) {
                if (boroughRefugeTotal.get(code) === undefined ||
                    boroughRefugeTotal.get(code) < 1) {
                    return null;
                } else if (originBorough.get(code) < 1) {
                    return ". Of these, <span id='arriving'>none*</span> were from " +
                        boroughNameById.get(code) + ". ";
                } else if (originBorough.get(code) == 1) {
                    return ", including <span id='arriving'>" +
                        originBorough.get(code) +
                        "*</span></b> woman from " + boroughNameById.get(
                            code) + ". ";
                } else {
                    return ", including <span id='arriving'>" +
                        originBorough.get(code) + "*</span> women from " +
                        boroughNameById.get(code) + ". ";
                }
            })
            .attr("class", "tooltip_text");
        ...
        var departuresHeading = d3.select("#departuresHeading")
            .append("text")
            .html(function(d) {
                if (refugeAccess.get(code) === undefined) {
                    return null;
                } else {
                    return "<span id='leavingH1'>Leaving to seek refuge**</span>";
                }
            })
            .attr("class", "tooltip_text");
        ...
        var accessingRefugeText = d3.select("#departuresInfo")
            ...
            .html(function(d) {
                if (refugeAccess.get(code) === undefined) {
                    return null;
                } else if (refugeAccess.get(code) == destinationLondon.get(
                        code)) {
                    return "<span id='leaving'>" + refugeAccess.get(code) +
                        " </span>" + " women from " + boroughNameById.get(
                            code) +
                        " were placed in a refuge in London. Of these, <span id='leaving'>" +
                        destinationLondon.get(code) + "</span> left " +
                        boroughNameById.get(code) + " to access a refuge.";
                } else if (originBorough.get(code) < 2) {
                    return "<span id='leaving'>" + refugeAccess.get(code) +
                        " </span>" + " woman from " + boroughNameById.get(
                            code) +
                        " were placed in a refuge in London. Of these, <span id='leaving'>" +
                        destinationLondon.get(code) + "</span> left " +
                        boroughNameById.get(code) + " to access a refuge.";
        ...
        var astrix = d3.select("#astrix")
            ...
            .html(function(d) {
                if (refugeAccess.get(code) === undefined) {
                    ...
                    return "<span id='arrivingAstrix'>*</span>This is likely to be higher due to some unknown London or unspecified locations.<br><span id='leavingAstrix'>**</span>Where relevant, this includes women who stay in the borough. There is no data for those women leaving London to seek refuge.";
                }
            })
            .attr("class", "astrix");
    ...
    function showInfoIn() {
        ...
        function showroutesIn(d) {
            ...
            var routesArrivingLondon = routes.filter(function(d) {
                if (d.lad_arrive_code == code && d.lad_depart_london ==
                    1 && d.lad_depart_name != "Unknown London" && d.lad_depart_code !=
                    code) {
                    return true;
                }
            });
            ...
            rectangle.selectAll("text")
                ...
                .append("text")
                .attr("x", function(d) {
                    return projection([+d.lon_depart - 0.043, +d.lat_depart +
                        0.03
                    ])[0];
                })
                .attr("y", function(d) {
            ...
            var totalRefugeText = d3.select("#totalRefugeArrivals")
                .append("text")
                .html(function(d) {
                    if (refugeTotal === undefined) {
                        return null;
                    } else if (refugeTotal < 1) {
                        return "There were no refuge spaces available in " +
                            boroughName + " at the end of March 2016.";
                    } else {
                        return "Of the <span id='arriving'>" + refugeTotal +
                            "</span> women who accessed refuge space in " +
                            boroughName + ":";
                    }
                })
                .attr("class", "tooltip_text");
            ...
            var originFromBoroughText = d3.select("#text1")
                .append("text")
                .html(function(d) {
                    if (refugeTotal === undefined || refugeTotal < 1) {
                        return null;
                    } else if (originBorough.get(code) < 1) {
                        return "No one came from " + boroughName;
                    } else if (originBorough.get(code) < 2) {
                        return originBorough.get(code) +
                            " woman came from " + boroughName;
                    } else {
                        return originBorough.get(code) +
                            " women came from " + boroughName;
                    }
                })
                .attr("class", "tooltip_text");

            var originBoroughPositions = [];
            for (var i = 1; i <= originBorough.get(code); i++) {
            ...
            var chartFromBorough = chart1Svg
                ...
                }) //move svgs along x-axis depending on pixel width
                .attr("class", "chart");
            ...
            var chartFromLondonText = d3.select("#text2")
                ...
                .html(function(d) {
                    if (refugeTotal === undefined || refugeTotal < 1) {
                        ...
                        return originLondon.get(code) + " women came from " +
                            routesArrivingLondon.length +
                            " other London boroughs";
                    } else if (originLondonUnknown.get(code) == 1 &&
                        routesArrivingLondon.length < 1) {
                        return originLondon.get(code) +
                            " women came from other London boroughs (including " +
                            originLondonUnknown.get(code) + " unknown)";
                    } else {
                        return originLondon.get(code) + " women came from " +
                            routesArrivingLondon.length +
                            " other London boroughs (including " +
                            originLondonUnknown.get(code) + " unknown)";
                    }
                })
                .attr("class", "tooltip_text");
            ...
            for (var j = 1; j <= originLondon.get(code); j++) {
            ...
            var chartFromOutsideLondonText = d3.select("#text3")
                .append("text")
                .html(function(d) {
                    if (originUK.get(code) === undefined) {
                        return null;
                    } else if (refugeTotal < 1) {
                        return null;
                    } else if (originUK.get(code) < 1) {
                        return "No one came from outside London";
                    } else {
                        return originUK.get(code) +
                            " women came from outside London";
                    }
                })
                .attr("class", "tooltip_text");

            var originUKPositions = [];
            for (var k = 1; k <= originUK.get(code); k++) {
            ...
            var originUnknownText = d3.select("#text4")
                ...
                .html(function(d) {
                    if (refugeTotal === undefined || refugeTotal < 1) {
                        return null;
                    } else if (originUnknown.get(code) < 1) {
                        return null;
                    } else if (originUnknown.get(code) == 1) {
                        return originUnknown.get(code) +
                            " woman came from an unspecified location ";
                    } else {
                        return originUnknown.get(code) +
            ...
            for (var l = 1; l <= originUnknown.get(code); l++) {
    ...
    function showInfoOut() {
        ...
        function showroutesOut(d) {
            ...
            //Routes leaving this borough to other london boroughs, not including routes within borough
            var routesDepartingLondon = routes.filter(function(d) {
                if (d.lad_depart_code == code && d.lad_depart_london ==
                    1) {
                    return true;
                }
            });
            ...
            destination_circles.selectAll("circle")
                ...
                .attr("class", "circle_destinations")
                .attr("cx", function(d) {
                    return projection([+d.lon_arrive + 0.025, +d.lat_arrive])[
                        0];
                })
                .attr("cy", function(d) {
                    return projection([+d.lon_arrive + 0.025, +d.lat_arrive])[
            ...
            labels.selectAll("text")
                ...
                .attr("x", function(d) {
                    return projection([+d.lon_arrive, +d.lat_arrive])[0];
                })
                .attr("y", function(d) {
                    return projection([+d.lon_arrive, +d.lat_arrive])[1];
                })
                .attr("dy", function(d) {
                    if (+d.lat_arrive < 51.5) {
                        return "2em";
                    } else {
                        return "-2em";
                    }
            ...
            var instructionJourneysOut = d3.select("#infowindowHeading")
                ...
                .html(function(d) {
                    if (boroughNameById.get(code) === undefined) {
                        return "<span class='tooltip_heading'>Domestic Abuse Migration Map</span> <br><span class='tooltip_text'> Women's journeys to access refuge in London, April 2015 - March 2016<br><span id='instructionLeavingH1'>Showing journeys leaving</span><br><b>Roll over</b> the London boroughs to discover the journeys women made.<br><b>Use the buttons above</b> the map to see all journeys or just those leaving.";
                    } else {
                        return null;
                    }
                })
            ...
            var helplineCalls = d3.select("#helplineCalls")
                ...
                .html(function(d) {
                    if (helplineCallsById.get(code) === undefined) {
                        ...
                        return "There were <span id='helplinecalls'>no</span> calls to the National Domestic Violence Helpline recorded from " +
                            boroughNameById.get(code) + ".";
                    } else {
                        return "There were <span id='helplinecalls'>" +
                            helplineCallsById.get(code) +
                            "</span> calls to the National Domestic Violence Helpline recorded from " +
                            boroughNameById.get(code) + ".";
                    }
                })
                .attr("class", "tooltip_text");
                ...
                .html(function(d) {
                    if (refugeAccess.get(code) === undefined) {
                        return null;
                            ...
                            code)) {
                        return "Of the <span id='leaving'>" + refugeAccess.get(
                                code) + " </span>" + " women placed in refuge in London who told us they came from " +
                            boroughNameById.get(code) +
                            "*: ";
                    } else if (originBorough.get(code) < 2) {
                        return "Of the <span id='leaving'>" + refugeAccess.get(
                                code) + " </span>" + " woman placed in refuge in London who told us she came from " +
                            boroughNameById.get(code) +
                            "*:";
                    } else {
                        return "Of the <span id='leaving'>" + refugeAccess.get(
            ...
            var chartToLondonText = d3.select("#text1")
                ...
                .html(function(d) {
                    if (refugeAccess.get(code) === undefined ||
                        refugeAccess.get(code) < 1) {
                        return null;
                    } else if (destinationLondon.get(code) < 1) {
                        return "No one travelled to other London boroughs";
                    } else {
                        return destinationLondon.get(code) +
                            " women travelled to " + routesDepartingLondon.length +
                            " other London boroughs";
                    }
                })
                .attr("class", "tooltip_text");
            ...
            for (var i = 1; i <= destinationLondon.get(code); i++) {
            ...
            var stayInBoroughText = d3.select("#text2")
                ...
                .html(function(d) {
                    if (refugeAccess.get(code) === undefined ||
                        refugeAccess.get(code) < 1) {
                        return null;
                    } else if (destinationBorough.get(code) < 1) {
                        return "No one stayed in " + boroughName;
                    } else {
                        return destinationBorough.get(code) +
                            " women stayed in " + boroughName;
                    }
                })
                .attr("class", "tooltip_text");

            var stayInBoroughPositions = [];
            for (var j = 1; j <= destinationBorough.get(code); j++) {
            ...
            var astrix = d3.select("#astrix2")
                ...
                .html(function(d) {
                    if (refugeAccess.get(code) === undefined) {
                        ...
                        return "*Note there is no data for those women leaving London to seek refuge. In addition, 254 women accessing refuge in London did not specify their borough of origin.";
                    }
                })
                .attr("class", "astrix");
...
function flyIn(d, origin, destination) {
    ...
    var routeIn = boroughs.append("path")
        .datum({
            type: "LineString",
            coordinates: [originFrom, destinationTo]
        })
        .attr("class", "routesin")
        .attr("d", path);
