...
// Micheal Keller's example of airline routes, namely the function to convert lattitude and longidtude to x and y coordinates  http://bl.ocks.org/mhkeller/f41cceac3e7ed969eaeb
//Michell Chandra Basic US state Map - http://bl.ocks.org/michellechandra/0b2ce4923dc9b5809922
// Scott Murray, Choropleth example from "Interactive Data Visualization for the Web" https://github.com/alignedleft/d3-book/blob/master/chapter_12/05_choropleth.html
...
//Jessica Hamel - for the SVG images loaded in to the charts http://blockbuilder.org/jessihamel/9648495
    ...
    var svg = d3.select("#map").append("svg")
        ...
        .attr("width", m_width)
        .attr("height", m_width * height / width); //78% of width
    ...
    function ready(error, uk, locations, routes) {
        ...
        //create the map
            boroughs.selectAll("path")
                ...
                .on("mouseout", function(d) { removeInfoAll(); });


    		d3.select("#infowindowHeading")
                .append("text")
                .html( "<span class='tooltip_heading'>Domestic Abuse Migration Map</span><br><span class='tooltip_text'> Women's journeys to access refuge in London, April 2015 - March 2016<br><span id='instructionAllH1'>Showing all journeys</span><br><b>Roll over</b> the London boroughs to discover the journeys women made.<br><b>Use the buttons above</b> the map to see just those arriving or leaving. </span>")
        ...
        function showInfoAll (d) {
            ...
            var routesArrivingLondon = routes.filter(function (d) { if (d.lad_arrive_code == code && d.lad_depart_london == 1 && d.lad_depart_code != code && d.lad_depart_name != "Unknown London") { return true; } });
            ...
            var routesUnknownOrOutsideLondon = routes.filter(function (d) { if ( d.lad_arrive_code == code && d.totalsNotLondon == 1 ) {return true ;}   });

            //Filter Routes: Include all routes
            var routesArrivingLondonAndOutside = routes.filter(function (d) { if ( d.lad_arrive_code == code && d.routesArrivingLondonAndOutside == 1 ) {return true ;}   });

            //Routes leaving this borough to other london boroughs, including routes within borough
            var routesDepartingLondon = routes.filter(function (d) { if (d.lad_depart_code == code && d.lad_depart_london == 1) { return true; } });
            //Routes leaving this borough to other london boroughs, not including routes within borough
            var routesDepartingLondonOther = routes.filter(function (d) { if (d.lad_depart_code == code && d.lad_depart_london == 1 && d.lad_arrive_code != code) { return true; } });
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                //showInfoAll mouseover: ALL ROUTES SUMMARY - TEXT. Each number has it's own style and color//
             ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            var instructionAllJourneys = d3.select("#infowindowHeading")
                .append("text")
                .html(function (d) { if (boroughNameById.get(code) === undefined) {return "<span class='tooltip_heading'>Domestic Abuse Migration Map</span> <br><span class='tooltip_text'> Women's journeys to access refuge in London, April 2015 - March 2016<br><span id='instructionAllH1'>Showing all journeys</span><br><b>Roll over</b> the London boroughs to discover the journeys women made.<br><b>Use the buttons above</b> the map to see just those arriving or leaving.</span>"; } else { return null; } })
                .attr("class", "instruction");
            ...
            var helplineCalls = d3.select("#helplineCalls")
                ...
                .html( function (d) { if (helplineCallsById.get(code) === undefined) { return null; }
                        else if (helplineCallsById.get(code) < 1 ) { return "<span id='helplinecalls'>No </span> calls to the National Domestic Violence Helpline recorded. "; }
                        else { return "<span id='helplinecalls'>" + helplineCallsById.get(code) + " </span> calls to the National Domestic Violence Helpline. "; }         }  )
                .attr("class", "tooltip_text");

            var arrivalsHeading = d3.select("#arrivalsHeading")
                .append("text")
                .html( function (d) { if (boroughRefugeTotal.get(code) === undefined ) { return null; }
                        else { return  "<span id='arrivingH1'>Arriving to access refuge</span>" ; }         }  )
                .attr("class", "tooltip_text");
            ...
            var arrivalsInfo = d3.select("#arrivalsInfo")
                .append("text")
                .html( function (d) { if (boroughRefugeTotal.get(code) === undefined) { return null; }
                        else if ( boroughRefugeTotal.get(code) < 1) { return "<span id='arriving'>No </span> women were able to seek refuge in this borough." ; }
                        else { return  "<span id='arriving'>" + boroughRefugeTotal.get(code) + " </span>" + "  women accessed a refuge in " + boroughNameById.get(code) ; }         }  )
                .attr("class", "tooltip_text");
            ...
            var originFromBoroughText = d3.select("#arrivalsInfo")
                ...
                .html( function (d) { if (boroughRefugeTotal.get(code) === undefined || boroughRefugeTotal.get(code) < 1 ) { return null; }
                        else if ( originBorough.get(code) < 1) { return ". Of these, <span id='arriving'>none*</span> were from " + boroughNameById.get(code) + ". " ; }
                        else if ( originBorough.get(code) == 1) { return ", including <span id='arriving'>" + originBorough.get(code) + "*</span></b> woman from " + boroughNameById.get(code) + ". " ; }
                        else { return  ", including <span id='arriving'>" + originBorough.get(code) + "*</span> women from " + boroughNameById.get(code) + ". " ; }         }  )
                .attr("class", "tooltip_text");
           ...
           var departuresHeading = d3.select("#departuresHeading")
            ...
            var accessingRefugeText = d3.select("#departuresInfo")
                .append("text")
                .html( function (d) { if (refugeAccess.get(code) === undefined) { return null; }
                    else if ( refugeAccess.get(code) == destinationLondon.get(code)) { return "<span id='leaving'>" + refugeAccess.get(code) + " </span>" + " women from " + boroughNameById.get(code) + " were placed in a refuge in London. Of these, <span id='leaving'>" + destinationLondon.get(code) + "</span> left " + boroughNameById.get(code) + " to access a refuge."; }
                    else if ( originBorough.get(code) < 2) { return "<span id='leaving'>" + refugeAccess.get(code) + " </span>" + " woman from " + boroughNameById.get(code) + " were placed in a refuge in London. Of these, <span id='leaving'>" + destinationLondon.get(code) + "</span> left " + boroughNameById.get(code) + " to access a refuge."; }
                    else { return "<span id='leaving'>" + refugeAccess.get(code) + " </span>" + " women from " + boroughNameById.get(code) + " were placed in a refuge in London. Of these, <span id='leaving'>" + destinationLondon.get(code) + "</span> left " + boroughNameById.get(code) + " to access a refuge."; }         } )
                .attr("class", "tooltip_text");

            var astrix = d3.select("#astrix")
                .append("text")
                .html(function (d) { if (refugeAccess.get(code) === undefined ) { return null; } else { return "<span id='arrivingAstrix'>*</span>This is likely to be higher due to some unknown London or unspecified locations.<br><span id='leavingAstrix'>**</span>Where relevant, this includes women who stay in the borough. There is no data for those women leaving London to seek refuge." ; } })
                .attr("class", "astrix");
        ...
        function showInfoIn () {
            ...
            d3.selectAll(".instruction")
	            ...
	            boroughs.selectAll("path")
	                ...
	                .on("mouseout", function(d) { removeInfoIn(); });

            var instructionJourneysIn = d3.select("#infowindowHeading")
                    .append("text")
	                .html( "<span class='tooltip_heading'>Domestic Abuse Migration Map</span> <br><span class='tooltip_text'> Women's journeys to access refuge in London, April 2015 - March 2016<br><span id='instructionArrivingH1'>Showing journeys arriving</span><br><b>Roll over</b> the London boroughs to discover the journeys women made.<br><b>Use the buttons above</b> the map to see all journeys or just those leaving.</span>")
                .attr("class", "instruction");
            ...
            function showroutesIn (d) {
                ...
                var routesArrivingLondon = routes.filter(function (d) { if (d.lad_arrive_code == code && d.lad_depart_london == 1 && d.lad_depart_name != "Unknown London" && d.lad_depart_code != code ) { return true; } });
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
             ...
             ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                //infoWindow heading, borough selected on Mouseover
                    ...
                    var instructionJourneysIn = d3.select("#infowindowHeading")
                        .append("text")
                        .html(function (d) { if (boroughNameById.get(code) === undefined) {return "<span class='tooltip_heading'>Domestic Abuse Migration Map</span> <br><span class='tooltip_text'> Women's journeys to access refuge in London, April 2015 - March 2016<br><span id='instructionArrivingH1'>Showing journeys arriving</span><br><b>Roll over</b> the London boroughs to discover the journeys women made.<br><b>Use the buttons above</b> the map to see all journeys or just those leaving.</span>"; } else { return null; } })
                        .attr("class", "instruction");

                    var totalRefugeText = d3.select("#totalRefugeArrivals")
                        .append("text")
                        .html( function (d) { if (refugeTotal === undefined ) { return null; }
                                else if (refugeTotal < 1 ) { return "There were no refuge spaces available in " + boroughName + " at the end of March 2016."; }
                                else { return  "Of the <span id='arriving'>" + refugeTotal + "</span> women who accessed refuge space in " + boroughName + ":" ; }         } )
                        .attr("class", "tooltip_text");
                ...
                // chart 2 - women from within London
                    var chartFromLondonText = d3.select("#text2")
                        .append("text")
                        .html( function (d) { if (refugeTotal === undefined || refugeTotal < 1 ) { return null; }
                                else if (originLondon.get(code) < 1 ) { return "No one came from other London boroughs"; }
                                else if (originLondonUnknown.get(code) < 1 ) { return originLondon.get(code) + " women came from " + routesArrivingLondon.length + " other London boroughs"; }
                                else if (originLondonUnknown.get(code) == 1 && routesArrivingLondon.length < 1 ) { return originLondon.get(code) + " women came from other London boroughs (including " + originLondonUnknown.get(code) + " unknown)" ; }
                                else { return originLondon.get(code) + " women came from " + routesArrivingLondon.length + " other London boroughs (including " + originLondonUnknown.get(code) + " unknown)" ; }         }  )
                        .attr("class", "tooltip_text");
                ...
                //Chart 4 - Unknown locations
                     ...
                     var originUnknownText = d3.select("#text4")
                        ...
                        .html( function (d) { if (refugeTotal === undefined || refugeTotal < 1) { return null; }
                                ...
                                else if ( originUnknown.get(code) == 1) { return originUnknown.get(code) + " women came from an unspecified location "; }
                                else { return  originUnknown.get(code) + " women came from unspecified locations" ; }         }  )
                        .attr("class", "tooltip_text");
        ...
        function showInfoOut () {
            ...
            boroughs.selectAll("path")
                ...
                .on("mouseout", function(d) { removeInfoOut(); });

            d3.select("#infowindowHeading")
                .append("text")
                .html("<span class='tooltip_heading'>Domestic Abuse Migration Map</span> <br><span class='tooltip_text'> Women's journeys to access refuge in London, April 2015 - March 2016<br><span id='instructionLeavingH1'>Showing journeys leaving</span><br><b>Roll over</b> the London boroughs to discover the journeys women made.<br><b>Use the buttons above</b> the map to see all journeys or just those leaving.</span>")
                .attr("class", "instruction");
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
             ...
             ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                //show instruction if borough is outside London
                var instructionJourneysOut = d3.select("#infowindowHeading")
                    .append("text")
                    .html(function (d) { if (boroughNameById.get(code) === undefined) {return "<span class='tooltip_heading'>Domestic Abuse Migration Map</span> <br><span class='tooltip_text'> Women's journeys to access refuge in London, April 2015 - March 2016<br><span id='instructionLeavinglH1'>Showing journeys leaving</span><br><b>Roll over</b> the London boroughs to discover the journeys women made.<br><b>Use the buttons above</b> the map to see all journeys or just those leaving."; } else { return null; } })
                    .attr("class", "instruction");

                var helplineCalls = d3.select("#helplineCalls")
                    .append("text")
                    .html( function (d) { if (helplineCallsById.get(code) === undefined) { return null; }
                            else if (helplineCallsById.get(code) < 1 ) { return "There were <span id='helplinecalls'>no</span> calls to the National Domestic Violence Helpline recorded from " + boroughNameById.get(code) + "."; }
                            else { return "There were <span id='helplinecalls'>" + helplineCallsById.get(code) + "</span> calls to the National Domestic Violence Helpline recorded from " + boroughNameById.get(code) + "."; }         }  )
                    .attr("class", "tooltip_text");
                ...
                var accessingRefugeText = d3.select("#departuresInfo")
                    .append("text")
                    .html( function (d) { if (refugeAccess.get(code) === undefined ) { return null; }
                        else if ( refugeAccess.get(code) == destinationLondon.get(code)) { return "Of the <span id='leaving'>" + refugeAccess.get(code) + " </span>" + " women from " + boroughNameById.get(code) + " placed in a refuge in London*: "; }
                        else if ( originBorough.get(code) < 2) { return "Of the <span id='leaving'>" + refugeAccess.get(code) + " </span>" + " woman from " + boroughNameById.get(code) + " placed in a refuge in London*:" ; }
                        else { return "Of the <span id='leaving'>" + refugeAccess.get(code) + " </span>" + " women from " + boroughNameById.get(code) + " placed in a refuge in London*:"; }         } )
                .attr("class", "tooltip_text");
                ...
                var chartToLondonText = d3.select("#text1")
                    ...
                    .html( function (d) { if (refugeAccess.get(code) === undefined  || refugeAccess.get(code) < 1 ) { return null; }
                            ...
                            else { return destinationLondon.get(code) + " women travelled to " + routesDepartingLondon.length + " other London boroughs" ; }         }  )
                    .attr("class", "tooltip_text");
                ...
                .append("text")
                .html(function (d) { if (refugeAccess.get(code) === undefined ) { return null; } else { return "*Note there is no data for those women leaving London to seek refuge." ; } })
                .attr("class", "astrix");

            }
        }

        function removeInfoIn () {
           ...
           d3.selectAll("circle")
            ...
            d3.selectAll(".placeLabelDeparture")
                .remove();

            d3.selectAll(".placeLabelOrigin")
                .remove();
...
//This next curly brace closes the main function (ready) click/buttons don't work outside of this. Other functions do and leave them outside of here to make them reuseable in and outside of the ready function.
}

//The following code is adapted from http://www.tnoda.com/blog/2014-04-02 to draw the arrows and create the transition along the paths, it has been merged with the function to convert the latitude and longitude in this example http://bl.ocks.org/mhkeller/f41cceac3e7ed969eaeb
